...
def level6(ENABLE_P2D,status,timer,locked, locked2,yInt, slope, quadratic,yIntLocked,slopeLocked, quadraticLocked, level): 
    '''
    Displays level 6.
    ...
    textSize(50)
    textAlign(LEFT)
    text("Level 6", displayWidth/6, displayHeight/6)
    pop()
    ...
    #Graph
    parabola(yInt,slope,quadratic,-5,9,9.5)
    
    #Check if correct
    if float("%.1f" % yInt) == -5 and float("%.1f" % slope) == 9 and float("%.1f" % quadratic) == 9.5:
        if level < 7:
            level = 7
