...
def level3(ENABLE_P2D,status,timer,locked, locked2,yInt, slope, yIntLocked,slopeLocked, level): 
    '''
    Displays level 3.
    ...
    textSize(50)
    textAlign(LEFT)
    text("Level 3", displayWidth/6, displayHeight/6)
    pop()
    ...
    #Graph
    linear(yInt,slope,1,1)
    
    #Check if correct
    if float("%.1f" % yInt) == 1 and float("%.1f" % slope) == 1:
        if level < 4:
            level = 4
